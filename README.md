...
**Total step loss** (see `dream_post/train_tabom.py`):

\[
\mathcal{L} = \mathcal{L}_{\text{CE}} + \lambda_{\text{rank}}\,\mathcal{L}_{\text{rank}}
\]

### Algorithm (one training step, pseudocode)

```
Input: trajectory with ground-truth response, decoding_order (and optional non_ltr_positions)
Hyperparams: supervision window M, rank window W_rank, margin m, λ_rank

# 1) Time-delay (TD) masking — align CE with a partial denoising state
Sample cut s_b per example (from trajectory; ling_coder may sample s from non_ltr steps)
Build input x_in: mask response tokens with decode_step ≥ s_b
CE targets: masked positions in [s_b, s_b + M)  →  L_CE

# 2) Forward student on x_in; get response logits

# 3) TABOM ranking — Boltzmann / pairwise surrogate on predictive entropy
For each batch item b with ≥2 supervised positions in the rank window:
  H_j = Shannon entropy of softmax(logits) at each masked response position j
  neg_e_j = -H_j   (higher ⇒ more confident / lower entropy)

  Restrict to trajectory steps in [s_b, s_b + W_rank)
  Sort positions by decode_step: earlier step = unmasked earlier at inference = "easier"

  For every pair (i, j) with step_i < step_j:
      L_pair += ReLU( neg_e_j - neg_e_i + m )   # want neg_e_i > neg_e_j

  L_rank_b = mean of L_pair over valid pairs in the window

L_rank = mean_b L_rank_b
L = L_CE + λ_rank * L_rank
```

Intuition: inference prefers unmasking **low-entropy (high-confidence)** tokens first; `L_rank` penalizes violations of that ordering inside the local window, approximating alignment with the inference Boltzmann prior without running multi-step decode during training.
